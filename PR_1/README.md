...
5.1. Basic Building Blocks Основная информация курса: 1. Способ
...
retyping 5 + 7 every time we need it, we can just create a new variable
that stores the  
result.
...
R’s built-in help  
files via the `?` command. For example, if you want more information on
the c() function,  
...
100 to make a  
vector of 100s. In other words, when you ask R to compute z \* 2 + 100,
what it really  
...
made a mistake and  
that you meant to add 1000 instead of 100. You could either re-type the
expression, or…
...
that you created  
earlier, but you can’t seem to remember if you named it my_div or myDiv.
You could try both  
